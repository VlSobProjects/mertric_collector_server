...
public abstract class AbstractMetricWriterController <
    ...
    @Operation(summary = "get validation fail metric collectors from group by id")
    @GetMapping("/collector/validationFail/{groupId}")
    public Mono<RG> getValidationFail(@PathVariable int groupId){
        return metricService.getValidationFailCollectorsFromGroup(groupId);
    }

    @Operation(summary = "get validation fail metric collectors from group by id")
    @RequestMapping(method = RequestMethod.DELETE,value = "/collector/validationFail/{groupId}")
    public Mono<RG> removeValidationFail(@PathVariable int groupId){
        return metricService.removeValidationFailCollectorsFromGroup(groupId);
    }
